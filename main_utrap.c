...
board_init(void)
{
	...
	gpio_set_mode(BOARD_PORT_LED_ACTIVITY,
		      	  GPIO_MODE_OUTPUT_50_MHZ,
		      	  GPIO_CNF_OUTPUT_PUSHPULL,
		      	  BOARD_PIN_LED_ACTIVITY);

	gpio_set_mode(BOARD_PORT_LED_BOOTLOADER,
		      	  GPIO_MODE_OUTPUT_50_MHZ,
		      	  GPIO_CNF_OUTPUT_PUSHPULL,
...
clock_init(void)
{
	 ...
	 * 12MHz (external) * 2 (multiplier) / 1 (PLLXTPRE_HSE_CLK) = 24MHz
	 */
...
led_toggle(unsigned led)
{
	switch (led) {
	case LED_ACTIVITY:
		BOARD_LED_ACTIVITY_TGL();
		break;

	case LED_BOOTLOADER:
		BOARD_LED_BOOTLOADER_TGL();
		break;
	}
}
